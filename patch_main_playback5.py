...
def modify():
    ...
    # The replace failed because of whitespace changes likely.
    # Let's extract exactly the `while let Some(event) = gui_event_rx.recv().await {` loop

    start_str = "            while let Some(event) = gui_event_rx.recv().await {"
    end_str = "            }"

    start_idx = content.find(start_str)
    # The end of the while loop is tricky to find via find(), let's use regex

    pattern = re.compile(r"            while let Some\(event\) = gui_event_rx\.recv\(\)\.await \{.*?\n            \}", re.DOTALL)

    new_loop = """            let mut cached_raw_audio: Vec<Vec<f32>> = Vec::new();
