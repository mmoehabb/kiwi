...
fn it_can_detect_wakewords_with_v2_file() {
    ...
    assert!((detected_wakewords[0].avg_score - 0.6495044).abs() < 1e-4);
    assert!((detected_wakewords[0].score - 0.7310586).abs() < 1e-4);
    assert!((detected_wakewords[1].avg_score - 0.5804737).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.721843).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_with_median_score_mode() {
    ...
    let detected_wakewords = run_detection_simulation(config, "/tests/resources/oye_casa_g.rpw");
    assert_eq!(detected_wakewords.len(), 2);
    assert!((detected_wakewords[0].avg_score - 0.64608675).abs() < 1e-4);
    assert!((detected_wakewords[0].score - 0.60123634).abs() < 1e-4);
    assert!((detected_wakewords[1].avg_score - 0.5288923).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.63968724).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_with_average_score_mode() {
    ...
    assert_eq!(detected_wakewords.len(), 2);
    assert!((detected_wakewords[0].avg_score - 0.64608675).abs() < 1e-4);
    assert!((detected_wakewords[0].score - 0.60458726).abs() < 1e-4);
    assert!((detected_wakewords[1].avg_score - 0.5750509).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.6313083).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_while_applying_band_pass_audio_filter() {
    ...
    let detected_wakewords = run_detection_simulation(config, "/tests/resources/oye_casa_g.rpw");
    assert_eq!(detected_wakewords.len(), 2);
    assert!((detected_wakewords[0].score - 0.6858197).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.66327363).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_while_applying_gain_normalizer_audio_filter() {
    ...
    assert_eq!(detected_wakewords.len(), 2);
    assert!((detected_wakewords[0].score - 0.7304294).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.71067876).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_while_applying_gain_normalizer_and_band_pass_audio_filters() {
    ...
    let detected_wakewords =
        run_detection_simulation_with_gains(config, "/tests/resources/oye_casa_g.rpw", 0.2, 5.);
    assert_eq!(detected_wakewords.len(), 2);
    assert!((detected_wakewords[0].score - 0.5775406).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.5828697).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_on_record_with_noise() {
    ...
    assert!((detected_wakewords[0].avg_score - 0.4676845).abs() < 1e-4);
    assert!((detected_wakewords[0].score - 0.527971).abs() < 1e-4);
    assert_eq!(detected_wakewords[0].counter, 24);
    assert!((detected_wakewords[1].avg_score - 0.32865646).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.48120698).abs() < 1e-4);
    assert_eq!(detected_wakewords[1].counter, 7);
    assert!((detected_wakewords[2].avg_score - 0.30807483).abs() < 1e-4);
    assert!((detected_wakewords[2].score - 0.5164661).abs() < 1e-4);
...
fn it_can_detect_wakewords_on_record_with_noise_using_filters() {
    ...
    assert_eq!(detected_wakewords.len(), 3);
    assert!((detected_wakewords[0].avg_score - 0.45496628).abs() < 1e-4);
    assert!((detected_wakewords[0].score - 0.5380342).abs() < 1e-4);
    assert_eq!(detected_wakewords[0].counter, 23);
    assert!((detected_wakewords[1].avg_score - 0.336222).abs() < 1e-4);
    assert!((detected_wakewords[1].score - 0.5001262).abs() < 1e-4);
    assert_eq!(detected_wakewords[1].counter, 5);
    assert!((detected_wakewords[2].avg_score - 0.3049497).abs() < 1e-4);
    assert!((detected_wakewords[2].score - 0.5189481).abs() < 1e-4);
...
fn it_can_detect_wakewords_using_trained_model() {
    ...
    assert!((detected_wakewords[0].score - 0.9997649).abs() < 1e-4);
    assert!((detected_wakewords[0].scores["ok_casa"] - 3.7506533).abs() < 1e-4);
    assert!((detected_wakewords[0].scores["none"] - (-16.83091)).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_using_trained_model_and_avg_score() {
    ...
    assert_eq!(detected_wakewords.len(), 1);
    assert_eq!(detected_wakewords[0].counter, 34);
    assert!((detected_wakewords[0].avg_score - 0.9997649).abs() < 1e-4);
    assert!((detected_wakewords[0].score - 0.9997649).abs() < 1e-4);
    assert!((detected_wakewords[0].scores["ok_casa"] - 3.7506533).abs() < 1e-4);
    assert!((detected_wakewords[0].scores["none"] - (-16.83091)).abs() < 1e-4);
}
...
fn it_can_detect_wakewords_in_eager_mode() {
    ...
    assert_eq!(detected_wakewords[0].avg_score, 0.);
    assert!((detected_wakewords[0].score - 0.9992142).abs() < 1e-4);
    assert!((detected_wakewords[0].scores["ok_casa"] - 23.990948).abs() < 1e-4);
    assert!((detected_wakewords[0].scores["none"] - 6.0654087).abs() < 1e-4);
}
#[test]
